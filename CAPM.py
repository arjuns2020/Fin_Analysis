...
class CAPM:
    ...
    def regression(self):
        ...
        beta, alpha = np.polyfit(self.data['m_returns'].values, self.data['s_returns'].values, deg=1)
    ...
    def plot_regression(self, alpha, beta):
        ...
        axis.scatter(self.data["m_returns"].values, self.data['s_returns'].values, label="Data Points")
        axis.plot(self.data["m_returns"].values, beta * self.data["m_returns"].values + alpha,
...
if __name__ == '__main__':
    capm = CAPM(['SQ', '^GSPC'], '2018-01-01', '2023-08-15')
    capm.initialize()
    capm.calculate_beta()
    capm.regression()
